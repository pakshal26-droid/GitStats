...
import { GitPullRequest, GitCommit, User, Calendar, BarChart, Search, Loader, Code, Star, GitFork, Building2 } from 'lucide-react';
...
const GitHub2 = () => {
  ...
  // Get commit stats per repository
  ...
  return (
    <div className="w-full py-10  max-w-4xl mx-auto">
        ...
        <div className="p-6">
          <div className="flex space-x-2 mb-6">
            ...
            <button 
              ...
              className={`px-4 py-2 rounded-md text-white flex items-center justify-center ${loading || !inputUsername.trim() ? 'bg-blue-300 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`}
            >
              {loading ? (
          ...
          <div className="p-6">
            <div className="flex space-x-2 mb-6">
              <input
                ...
                className="flex-grow px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
              />
              <button 
                onClick={fetchOrgPRs}
                disabled={loading || !inputOrgName.trim() || !username}
                className={`px-4 py-2 rounded-md text-white flex items-center justify-center ${loading || !inputOrgName.trim() || !username ? 'bg-indigo-300 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700'}`}
              >
                <Search size={16} />
                <span className="ml-2">Search Org</span>
              </button>
            </div>
            ...
            {matchingPRs.length > 0 && (
              <div>
                <h3 className="text-sm font-medium text-gray-600 mb-2">
                  Found {matchingPRs.length} matching PRs in {inputOrgName}
                </h3>
            ...
            {orgPRData && matchingPRs.length === 0 && (
              <div className="text-gray-500">
                No matching PRs found for {username} in {inputOrgName}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-lg overflow-hidden border border-gray-200">
          ...
          <div className="p-6">
            <ul className="space-y-4">
              {topRepos.map((repo) => (
                <li key={repo.name} className="border border-gray-100 rounded-md p-4 hover:bg-gray-50">
                  <div className="font-medium text-blue-600"><a href={repo.url} target='_blank'>{repo.name}</a></div>
                  {repo.description && (
                    <div className="text-sm text-gray-600 my-1">{repo.description}</div>
                  )}
